...
int rn_tcpserver_open(const char *addr, int port)
{
	...
	if (bind(socketfd, (struct sockaddr *) &serveraddr, sizeof(struct sockaddr_in)) != 0) {
		close(socketfd);
		rtsd_error("can't bind tcp socket:%s", strerror(errno));
		return -2;
	}

	if (listen(socketfd, 5) != 0) {
		close(socketfd);
		rtsd_error("can't listen tcp socket:%s", strerror(errno));
		return -3;
	}
...
int rn_udpserver_open(const char *addr, int port)
{
	...
	if (bind(socketfd, (struct sockaddr *) &serveraddr, sizeof(struct sockaddr_in)) != 0) {
		close(socketfd);
		rtsd_error("can't bind udp socket:%s", strerror(errno));
		return -2;
	}
...
int rn_udpmulticast_open(const char *addr, int port)
{
	...
	if ((socketfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		rtsd_error("can't open multucast socket:%s", strerror(errno));
		return -1;
	}

#ifdef SO_REUSEPORT
	if (setsockopt(socketfd, SOL_SOCKET, SO_REUSEPORT, (const char*) &reuse, sizeof(reuse)) < 0) {
		rtsd_error("can't setting multucast socket:%s", strerror(errno));
	}
#endif
#ifdef IP_MULTICAST_LOOP
	if (setsockopt(socketfd, IPPROTO_IP, IP_MULTICAST_LOOP, (const char*) &loop, sizeof(loop)) < 0) {
		rtsd_error("can't setting multicast socket:%s", strerror(errno));
	}
#endif
